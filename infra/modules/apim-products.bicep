...
@description('API names to add to every product so a product subscription is valid for all of them. Pass only the APIs actually deployed.')
...
output productNames array = [for (t, i) in productTiers: products[i].name]
